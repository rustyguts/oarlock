<script lang="ts">
	...
	async function toggleEnabled(wf: Workflow, next: boolean) {
		const prev = wf.is_enabled;
		wf.is_enabled = next;
		try {
			await api.patchWorkflow(wf.id, { is_enabled: next });
			error = '';
		} catch (e) {
			wf.is_enabled = prev;
			error = e instanceof Error ? e.message : String(e);
	...
	async function confirmDelete() {
		...
		try {
			...
			if (e instanceof ApiError && e.workflows?.length) {
				error = `${e.message}: ${e.workflows.join(', ')}`;
			} else {
				error = e instanceof Error ? e.message : String(e);
			}
		}
				...
				{#each workflows as wf (wf.id)}
					<div class="hover:bg-muted/50 flex items-center justify-between px-4 py-3">
						<a
							...
							class="min-w-0 flex-1 transition-opacity {wf.is_enabled ? '' : 'opacity-55'}"
						>
						...
						<div class="ml-4 flex items-center gap-3">
							<Switch
								...
								onCheckedChange={(v) => toggleEnabled(wf, v)}
								aria-label={wf.is_enabled ? 'Disable workflow' : 'Enable workflow'}
								title="Disabled workflows won't be started by triggers (manual runs still work)"
							/>
...
<ConfirmDialog
	...
	description={pendingDelete
		? `"${pendingDelete.name}" and all its runs will be permanently deleted.`
		: ''}
	confirmText="Delete"
