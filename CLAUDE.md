...
**One image** (root `Dockerfile`): the web UI builds to a static SPA, is copied into `engine/internal/webui/dist/`, and the Go binary embeds and serves it. `OARLOCK_MODE` selects the role at runtime — `all` (default: UI + API + workers), `api` (UI + API, inserts jobs only), `worker` (workers/reaper/scheduler + `/healthz` only). Schema + River migrations run under a Postgres advisory lock so concurrently booting replicas are safe. The Helm chart lives in `deploy/chart/oarlock` (mode=simple|scalable).

**Releases** (`.github/workflows/release.yml`, release-it): pushes to main cut dev prereleases (tag `vX.Y.Z-dev.N`, image tags `X.Y.Z-dev.N` + `dev`; no bump commit — versions derive from git tags via `npm.ignoreVersion`); production releases are workflow_dispatch with increment `patch|minor` only (v0 repo) and produce a release commit (`scripts/set-version.sh` syncs `package.json`/`Chart.yaml`/main.go's version const), tag, GitHub Release, and image tags `X.Y.Z` + `latest`. Image publishing lives inside the release workflow because `GITHUB_TOKEN`-pushed tags never trigger other workflows. `docker.yml` is a PR-only smoke build. The root `package.json` exists solely for release-it.
...
**Auth** (`internal/api/auth.go`, `users.go`, `password.go`; project.md §7): password login with argon2id hashes. First run is *setup mode* — while no user has a password, `WithAuth` 401s with `setup_required` and `POST /v1/setup` claims the migration-seeded owner in place (first account = admin). Signup is closed; admins create users (`/v1/users`, admin-only via `requireAdmin`) with `must_change_password`. Sessions keep the sha256-hashed-token cookie mechanism; the old auto-login bootstrap is gone. `setupConfigured` is a process-level `atomic.Bool` cache — DB-backed tests must `setupConfigured.Store(false)` after truncating users (see `newTestServer`).

**Tenancy**: every API request resolves a workspace from a session cookie or an `Authorization: Bearer oak_…` token — `WithAuth` prefers the token. Token principals are **member tier** and hard-excluded from the admin surface (`requireAdmin` always fails them), so a leaked key drives workflows but can't manage users/tokens. `authInfo.userIDOrNil()` records NULL `created_by` for token-created rows. All queries filter by `workspace_id` via `s.workspace(r)`. Never join across workspaces (hard rule 7). CORS is credentialed against an explicit origin allowlist (`OARLOCK_ALLOWED_ORIGINS`, default `http://localhost:3001`) — never wildcard. Request bodies are capped at 1MB (`api.MaxBody`) and 5xx responses are sanitized to `{"error":"internal error"}`.
